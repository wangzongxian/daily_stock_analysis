...
**Scope（本轮实现范围）：**`#1381` 仅覆盖后端大盘上下文注入、当日/目标交易日复用控制与保守护栏；本轮不包含独立 API、Web 阶段结果独立展示、四阶段日报结构化持久化或新增日报状态表；未落地项已移至后续计划/Issue。范围对应目录与入口为 `main.py`（调度与 `--no-market-review`）、`src/core/pipeline.py`、`src/services/daily_market_context.py`、`src/analyzer.py`、`src/agent/*.py`、`src/daily_market_context_guardrail.py`。
...
**兼容性证据：**本轮未新增 provider/model/base_url 迁移逻辑、默认模型回退/覆盖逻辑或配置清理逻辑；核验路径为：仅通过 `src/core/pipeline.py` / `src/services/daily_market_context.py` / `main.py` 读取并复用既有 `LLM` 配置链路，不新增/改写 `.env`、`系统设置` 或数据库迁移；回滚路径同样为常规发布回滚，必要时配合重启/清理 runtime 覆盖项恢复用户保存值（与现网既有说明一致）。
